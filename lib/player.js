...
(function (global) {
  ...
  function getSourceType(url) {
    ...
    // Поддержка готовой embed-ссылки с vkvideo.ru (video_ext.php)
    if (u.includes('vkvideo.ru') && u.includes('video_ext.php') && u.includes('oid=') && u.includes('id=')) {
      return 'vk-video-proxy-direct';
    }

    // Поддержка "человеческой" ссылки: vkvideo.ru/video-123_456
    if (u.includes('vkvideo.ru/video')) return 'vk-video-proxy';

    // Официальные ссылки ВКонтакте — embed без hash не работает
    if (u.includes('vk.com/video')) return 'vk-video-official';
  ...
  function renderAudio(rawUrl) {
    ...
    // VK Video: готовая ссылка на video_ext.php от vkvideo.ru
    if (type === 'vk-video-proxy-direct') {
      try {
        // Проверим, что URL валиден
        new URL(url);
        return `
          <div class="audio-player">
            <iframe src="${url}" 
            ...
            <a href="${url}" target="_blank" class="action-link">Открыть на vkvideo.ru</a>
          </div>`;
      } catch (e) {
    ...
    // VK Video: обычная ссылка вида https://vkvideo.ru/video-123_456
    ...
    // Официальное видео ВКонтакте (vk.com) — embed без hash не работает → только ссылка
